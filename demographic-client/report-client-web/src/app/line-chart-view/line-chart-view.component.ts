...
export class LineChartViewComponent implements OnInit {
  @ViewChild("baraccuracychart", { static: true })
  baraccuracychart: ChartComponent;
  ...
  series: ApexAxisChartSeries;
  constructor(
    private fileservice: FileReadingService,
  ...
  ngOnInit(): void {
    ...
    this.httpclient.get(this.source, { responseType: "text" }).subscribe(
      (data) => {
        ...
        for (let index = 1; index < csvToRowArray.length - 1; index++) {
          ...
          if (!namefile.has(row[0])) {
            namefile.set(row[0], [
              [Number(row[1]).toFixed(3)],
              [Number(row[2].replace("\r", "")).toFixed(3)],
            ]);
          } else {
            namefile.get(row[0])[0].push(Number(row[1]).toFixed(3));
            namefile
              .get(row[0])[1]
              .push(Number(row[2].replace("\r", "")).toFixed(3));
          }
        }
  ...
  changeCharttoBar() {
    var btn = document.getElementsByClassName(this.radioName);

    if (!btn[0].classList.contains("btn-primary")) {
      btn[0].classList.add("btn-primary");
    }

    if (btn[1].classList.contains("btn-primary")) {
  ...
  changeCharttoLine() {
    ...
    btn[1].classList.add("btn-primary");
    if (btn[0].classList.contains("btn-primary")) {
